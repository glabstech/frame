...
class ACF {
	...
	function initialize() {
		...
		$this->define( 'ACF_PATH', get_template_directory(__FILE__).'/admin/lib/acfpro/'/*plugin_dir_path( __FILE__ )*/ );
		$this->define( 'ACF_BASENAME', '/admin/lib/acfpro/'/*plugin_basename( __FILE__ )*/ );
		...
		$this->settings = array(
			...
			'path'						=> ACF_PATH,
			'file'						=> __FILE__,
			'url'						=> get_stylesheet_directory_uri(__FILE__).'/'/*plugin_dir_url( __FILE__ )*/,
			'show_admin'				=> true,
			'show_updates'				=> true,
			'stripslashes'				=> false,
			'local'						=> true,
	...
	acf_update_setting( 'url', get_stylesheet_directory_uri(__FILE__).'/'/*plugin_dir_url( __FILE__ )*/ );
